...
import com.mayhem.rs2.entity.player.net.out.impl.SendLoginResponse;
import com.mayhem.rs2.entity.player.net.out.impl.SendLogout;
import com.mayhem.rs2.entity.player.net.out.impl.SendMapRegion;
...
import com.mayhem.rs2.entity.player.net.out.impl.SendSkillGoal;
...
public class Player extends Entity {
	...
	public boolean login(boolean starter) throws Exception {
		...
		send(new SendString("</col>Valius Bucks: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		send(new SendMessage("<img=8>@blu@Welcome to Valius!"));
		send(new SendMessage("<img=8>@blu@Make sure you register on our active community forums: www.valius.org/community"));
		send(new SendMessage("<img=8>@blu@Don't forget to ::Vote every 12 hours for great rewards and to help the server grow!"));
		send(new SendMessage("<img=8>@blu@Type ::Donate to visit our online store! (25% off everything in the store!)"));
		
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>Valius</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
